...
// Static JSON null value for singleton pattern
static struct json json_null_value = {JSON_NULL, {0}};
...
void json_free(struct json *json) {
    if (!json || json == &json_null_value) return;
...
void json_array_push(struct json *array, struct json *value) {
    if (!array || !value || !json_isarray(array)) return;
...
struct json *json_object_remove(struct json *object, const char *key) {
    if (!object || !key || !json_isobject(object)) return NULL;
